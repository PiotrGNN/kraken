"""
OKX exchange connector implementation for DeepAgent Kraken trading bot.

This module provides a connector for the OKX API using the CCXT library.
...
def handle_rate_limit(func):
    """
    Decorator to handle rate limiting errors from OKX API.
...
class OKXConnector:
    """
    Connector for the OKX API using CCXT.
    
    This class provides methods for interacting with the OKX API,
    ...
    async def initialize(self):
        ...
        logger.info(f"Initializing OKX connector (testnet: {self.testnet})")
        
        # Configure CCXT OKX client
        options = {
    ...
    def _ensure_initialized(self):
        ...
        if not self.initialized or not self.client:
            raise RuntimeError("OKX connector not initialized. Call initialize() first.")
    ...
    def _handle_ccxt_error(self, e: Exception):
        ...
        if isinstance(e, ccxt.AuthenticationError):
            ...
            raise Exception(f"OKX authentication failed: {error_msg}")
        elif isinstance(e, ccxt.InsufficientFunds):
            logger.error(f"Insufficient funds: {error_msg}")
            raise Exception(f"Insufficient funds on OKX: {error_msg}")
        elif isinstance(e, ccxt.InvalidOrder):
            logger.error(f"Invalid order: {error_msg}")
            raise Exception(f"Invalid order on OKX: {error_msg}")
        elif isinstance(e, ccxt.ExchangeError):
            logger.error(f"Exchange error: {error_msg}")
            raise Exception(f"OKX exchange error: {error_msg}")
        elif isinstance(e, ccxt.NetworkError):
            logger.error(f"Network error: {error_msg}")
            raise Exception(f"Network error connecting to OKX: {error_msg}")
        else:
            logger.error(f"Unexpected error: {error_msg}")
            raise Exception(f"Unexpected error with OKX: {error_msg}")
    ...
    async def is_healthy(self) -> bool:
        ...
        try:
            ...
            logger.error(f"OKX health check failed: {str(e)}")
            return False
    ...
    def get_positions(self, symbol: Optional[str] = None) -> Dict:
        ...
        try:
            # OKX requires symbol to be in their specific format
            formatted_symbol = symbol.replace('/', '-') if symbol else None
            
            # Fetch positions
            positions = self.client.fetch_positions(formatted_symbol)
